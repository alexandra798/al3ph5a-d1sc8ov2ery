...
class AlphaEnvWrapper(gymnasium.Wrapper):
    state: np.ndarray
    env: AlphaEnvCore
    ...
    def __init__(self, env: AlphaEnvCore):
        super().__init__(env)
        self.action_space = gymnasium.spaces.Discrete(SIZE_ACTION)
        self.observation_space = gymnasium.spaces.Box(low=0, high=SIZE_ALL - 1, shape=(MAX_EXPR_LENGTH, ), dtype=np.uint8)
