...
class ts_std(RollingOperator):
    def _apply(self, operand: Tensor) -> Tensor: 
        # 添加最小样本数检查，避免degrees of freedom <= 0
        if operand.shape[-1] <= 1:
            # 如果样本数不足，返回0或很小的值
            return torch.zeros_like(operand[..., 0])
        return operand.std(dim=-1, unbiased=False)  # 使用unbiased=False避免n-1问题

class ts_ir(RollingOperator):
    def _apply(self, operand: Tensor) -> Tensor: 
        if operand.shape[-1] <= 1:
            return torch.zeros_like(operand[..., 0])
        mean_val = operand.mean(dim=-1)
        std_val = operand.std(dim=-1, unbiased=False)
        # 避免除零
